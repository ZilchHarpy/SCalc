...
Contém a classe ValidadorDados para centralizar todas as validações
...
class ValidadorDados:
    ...
    def validar_dataframe(df: pd.DataFrame, nome: str = "DataFrame") -> bool:
        ...
        Raises:
            DadosInvalidosException: Se o DataFrame for inválido
        """
        if not isinstance(df, pd.DataFrame):
            raise DadosInvalidosException(f"{nome} deve ser um pd.DataFrame")
        
        if df.empty:
            raise DadosInvalidosException(f"{nome} está vazio")
        
        if len(df.columns) == 0:
            raise DadosInvalidosException(f"{nome} não possui colunas")
    ...
    def validar_arquivo_excel(caminho: str) -> bool:
        """
        Valida se arquivo Excel existe e é acessível
        ...
        Raises:
            ArquivoInvalidoException: Se arquivo for inválido
        """
        import os
        
        if not os.path.exists(caminho):
            raise ArquivoInvalidoException(f"Arquivo não encontrado: {caminho}")
        ...
        if tamanho_mb > Config.Validacao.MAX_TAMANHO_ARQUIVO_MB:
            raise ArquivoInvalidoException(
                f"Arquivo excede tamanho máximo de {Config.Validacao.MAX_TAMANHO_ARQUIVO_MB}MB"
            )
    ...
    def validar_dados_numericos(
        ...
        """
        Valida se série contém dados numéricos
        
        Args:
            serie: Série a validar
            nome_coluna: Nome da coluna para mensagens
            
        Returns:
            pd.Series: Série convertida para numérico
            
        Raises:
            DadosNaoNumericosException: Se contiver dados não numéricos
        """
        try:
            ...
            if dados_numericos.isna().all():
                raise DadosNaoNumericosException(
                    f"Coluna '{nome_coluna}' contém apenas valores não numéricos"
                )
            
            nans_removidos = serie.isna().sum() - dados_numericos.isna().sum()
            if nans_removidos > 0:
                logger.warning(
                    f"Coluna '{nome_coluna}': {nans_removidos} valores não numéricos ignorados"
                )
    ...
    def validar_medicoes_minimas(
        ...
        """
        Valida se há mínimo de medições recomendado
        
        Args:
            dados: Dicionário de dados
            minimo: Mínimo de medições (padrão: Config.Estatistica.MIN_MEDICOES_RECOMENDADO)
            
        Returns:
            bool: True se há medicões suficientes
        ...
        if total_medicoes < minimo:
            raise DadosInsuficientesException(
                f"Mínimo de {minimo} medições recomendado, encontrado: {total_medicoes}"
            )
        
        logger.info(f"Validação de medições: {total_medicoes} encontradas (mínimo: {minimo})")
        return True
    ...
    def validar_tamanho_arquivo(
        ...
        """
        Valida se arquivo está dentro dos limites de tamanho
        
        Args:
            df: DataFrame
            max_linhas: Máximo de linhas (padrão: Config)
            max_colunas: Máximo de colunas (padrão: Config)
